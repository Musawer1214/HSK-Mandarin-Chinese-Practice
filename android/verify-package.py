...
root=Path(__file__).resolve().parent;apk=root/'release/HSK-Recall-1.3.1.apk'
assert apk.exists(),apk
if apk.exists():
 with zipfile.ZipFile(apk) as z:
  bad=z.testzip();assert bad is None,bad
  audio=[n for n in z.namelist() if n.startswith('assets/audio/')];assert len(audio)==1200
  vocabulary=json.loads(z.read('assets/vocabulary.js').decode().split('=',1)[1].rstrip(';'))
  assert len(vocabulary)==1200 and len({w['id'] for w in vocabulary})==1200
  assert {level:sum(w['level']==level for w in vocabulary) for level in range(1,5)}=={1:150,2:150,3:300,4:600}
  ...
  print('APK integrity, 1200 entries/clips, and unchanged HSK 1-3 IDs verified. No saved user data or signing keys included.')
